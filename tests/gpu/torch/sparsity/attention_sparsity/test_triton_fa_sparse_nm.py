...
"""GPU tests for N:M structured sparsity on the Triton flash attention kernel."""
...
class TestSparseNM:
    """N:M structured sparsity behavior on attention scores."""
    ...
    def test_sink_tokens_preserve_early_kv(self, n, m):
        """num_sink_tokens keeps early KV blocks dense, reducing error vs fully sparse."""
        q, k, v, locs, lens = self._make_inputs(seq_len=512)
        scale = 1.0 / (64**0.5)
        out_dense = attention(q, k, v, locs, lens, 512, softmax_scale=scale)
