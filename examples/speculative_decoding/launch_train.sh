...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --eagle_base_lora_preservation_loss_weight*)
      ...
      ;;
    --eagle_base_lora_lm_loss_weight*)
      if [[ "$1" != *=* ]]; then shift; fi
      EAGLE_BASE_LORA_LM_LOSS_WEIGHT="${1#*=}"
      ;;
...
EAGLE_BASE_LORA_LM_LOSS_WEIGHT=${EAGLE_BASE_LORA_LM_LOSS_WEIGHT:-1.0}
...
if [[ "$EAGLE_BASE_LORA" == "True" ]]; then
  LORA_ARGS="--eagle_base_lora True \
             ...
             --eagle_base_lora_lm_loss_weight $EAGLE_BASE_LORA_LM_LOSS_WEIGHT \
