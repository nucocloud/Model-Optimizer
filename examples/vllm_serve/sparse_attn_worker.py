...
``SparseAttnWorker``: applies sparse attention via ``mtsa.sparsify()``.
...
Usage:
    SPARSE_ATTN_CFG=SPARSE_SOFTMAX_DEFAULT vllm serve model \\
        --worker-cls examples.vllm_serve.sparse_attn_worker.SparseAttnWorker
"""
...
def _build_sparse_config(env_config: dict[str, Any]) -> dict | None:
    ...
    raise ValueError(f"Unknown sparse config: {cfg_name}. Set SPARSE_ATTN_CFG to 'default' or a valid preset name.")
...
def _apply_sparse_to_attention_modules(model, sparse_cfg: dict):
    ...
    for name, module in model.named_modules():
        ...
        if sparsity_n > 0:
            ...
            sparse_kw["num_sink_blocks"] = layer_cfg.get("num_sink_blocks", 0)
            sparse_kw["dense_window_blocks"] = layer_cfg.get("dense_window_blocks", 1)
...
class SparseAttnWorker(BaseWorker):
    """vLLM worker that applies sparse attention via direct module patching."""

    def compile_or_warm_up_model(self) -> None:
        """Apply sparse attention config before warm-up."""
